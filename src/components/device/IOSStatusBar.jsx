...
/** iOS status bar: clock on the left, signal / wifi / battery on the right. */
export default function IOSStatusBar({ dark = false, time = '9:41' }) {
  const tint = dark ? '#fff' : '#000';

  return (
    <div className={`${styles.bar} ${dark ? styles.dark : ''}`} aria-hidden="true">
      <div className={styles.slot}>
        <span className={styles.time}>{time}</span>
      </div>
      <div className={`${styles.slot} ${styles.icons}`}>
        {/* cellular */}
